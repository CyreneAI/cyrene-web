...
const handleTwitterLink = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/auth/twitter/login', {
      ...
      body: JSON.stringify({ 
        walletAddress,
        returnUrl: '/agents' // Explicitly set the correct return URL
      }),
    });
    ...
    if (!response.ok) {
      ...
      console.error('Twitter link error response:', errorData);
      throw new Error(errorData.error || 'Failed to initiate Twitter login');
    }

    const { authUrl, message } = await response.json();
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to link Twitter account';
